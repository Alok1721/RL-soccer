    ...
    while True:
        ...
        if marker_corners:
            ...
            for i in range(len(marker_ids)):
                ...
                current_id = marker_ids[i][0]
                print(current_id)
                if current_id in team2:
                    current_team, opponent_team = team2, team1
                try:
                    agent = Agents[current_id]
                except Exception as e:
                    print("Couldn't Get Agent ka ID", current_id,Agents)
                    return
                ...
                for endpoint in endpoints:
                    ...
                    observation.append(RayObv)
                    # print(f"for agent{i} ray{rayid} 1 hot bit-->",RayObv)
                
                agent.AddObservation(observation)
                print("called ADD observation",current_id)

                for endpoint in adjusted_endpoints:
                    cv2.arrowedLine(frame, (int(centre[0]), int(centre[1])), (int(endpoint[0]), int(endpoint[1])), (255, 0, int(255 ^ marker_ids[i])), 2)
                print("="*20)


        frame,ball_centre,_=ball_tracking(frame)
